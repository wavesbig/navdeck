...
import {resolveCardUrl, resolveAutoUrl, probeUrl, probeUrls} from './network';
import type {Card, NetworkMode} from '@/types';

function makeCard(overrides: Partial<Card> = {}): Pick<Card, 'internalUrl' | 'externalUrl'> {
  return {
...
describe('resolveAutoUrl', () => {
  it('外网可达时返回外网', () => {
    const result = resolveAutoUrl(makeCard(), {internal: true, external: true});
    expect(result.source).toBe('external');
  });

  it('外网不可达但内网可达时返回内网', () => {
    const result = resolveAutoUrl(makeCard(), {internal: true, external: false});
    expect(result.source).toBe('internal');
    expect(result.url).toBe('http://192.168.1.10:8096');
  });

  it('内外网都不可达时兜底外网', () => {
    const result = resolveAutoUrl(makeCard(), {internal: false, external: false});
    expect(result.source).toBe('external');
  });
});
